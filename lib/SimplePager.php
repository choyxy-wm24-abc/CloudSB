...
class SimplePager {
    ...
    public function html($href = '', $attr = '') {
        ...
        echo "<a href='?page=1&user_id=$href'>First</a>";
        echo "<a href='?page=$prev&user_id=$href'>Previous</a>";

        for ($p = 1; $p <= $this->page_count; $p++) {
            $c = $p == $this->page ? 'active' : '';
            echo "<a href='?page=$p&user_id=$href' class='$c'>$p</a>";
        }

        echo "<a href='?page=$next&user_id=$href'>Next</a>";
        echo "<a href='?page=$this->page_count&user_id=$href'>Last</a>";
        echo "</nav>";
    }
}